...
static struct
{
    volatile uint8_t game:1;
    volatile uint8_t music:1;
} intFlag;
...
void game_init(void)
{
    ...
    adc_setCallbacks(&rand_feed, &dummy);
    adc_init();
...
void game_run(void)
{
    ...
    for (;;)
    {
        ...
        {
            PORTL = intFlag.game;
            if (intFlag.game == 1 || workLeft.game != DONE)
            {
                if (START == game_state)
            ...
            if (intFlag.music == 1 || workLeft.music != DONE)
            {
                //workLeft.music = music_play();
                intFlag.music = 0;
            }
        } while (intFlag.game == 1 || workLeft.game != DONE || 
                 intFlag.music == 1 || workLeft.music != DONE);
            ...
            if (screenImage.walls[w].points[0] > MAX_GAP_WIDTH)
            {
                for (uint8_t p = 0; p < WALL_POINTS; p += 2)
                {
                    ...
                    if (screenImage.walls[w].yPos == screenImage.ball.y+BALL_RADIUS+1)
                    {
                        ...
                        if (p == 0)
                        {
                            if (screenImage.walls[w].points[0] >= screenImage.ball.x+BALL_RADIUS)
                            {
                                yCollision = 1;
                                break;
                            }
                    ...
                    if (p == 0)
                    {
                        if (screenImage.walls[w].points[0] == screenImage.ball.x-BALL_RADIUS-1)
                        {
                            xCollisionL = 1;
                            break;
                        }
            ...
            else
            {
                for (uint8_t p = 0; p < WALL_POINTS; p += 2)
                {
                    ...
                    if (screenImage.walls[w].yPos == screenImage.ball.y+BALL_RADIUS+1)
                    {
                        ...
                        if (p == WALL_POINTS-1)
                        {
                            if (screenImage.walls[w].points[WALL_POINTS-1] <= screenImage.ball.x+BALL_RADIUS)
                            {
                                yCollision = 1;
                                break;
                            }
                    ...
                    if (p == WALL_POINTS-1)
                    {
                        if (screenImage.walls[w].points[WALL_POINTS-1] == screenImage.ball.x+BALL_RADIUS+1)
                        {   
                            xCollisionR = 1;
                            break;
                        }
